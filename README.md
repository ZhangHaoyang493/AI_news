...
我们目前已经配置好了 GitHub Actions 工作流(`.github/workflows/daily_news.yml`)，它会在每天中国北京时间上午 9:00（UTC时间 1:00）自动运行。如果您想启用它推送到飞书，只需按照以下步骤设置：
...
完成配置后，Action 就能够自动抓取资讯、翻译并在每天早上将新鲜结果直接发在您的飞书群里啦！您也可以在 GitHub的 "Actions" 页面手动点击 "Run workflow" 测试一次。
